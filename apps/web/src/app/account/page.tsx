import type { Metadata } from "next";
import { auth, signIn, signOut } from "@/auth";
import AccountDashboard from "./AccountDashboard";

export const metadata: Metadata = {
  title: "My Account",
  description: "Your Agent Artifacts library, saved products, and purchase history.",
};

export default async function AccountPage() {
  const session = await auth();

  return (
    <>
      <div style={{ background: "white", borderBottom: "1px solid var(--border)", padding: "3rem 0" }}>
        <div className="container" style={{ maxWidth: "760px" }}>
          <div className="section-label">Account</div>
          <h1 style={{ marginBottom: "0.4rem" }}>
            {session ? `Hi, ${session.user?.name?.split(" ")[0] ?? "there"}` : "My account"}
          </h1>
          <p style={{ color: "var(--ink-muted)" }}>
            {session
              ? "Your saved library, purchased products, and download history."
              : "Sign in to access your library and purchase history."}
          </p>
        </div>
      </div>

      <div className="section-sm">
        <div className="container" style={{ maxWidth: "760px" }}>
          {session ? (
            <>
              <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "1.5rem" }}>
                <form action={async () => { "use server"; await signOut({ redirectTo: "/account" }); }}>
                  <button type="submit" className="btn btn-outline btn-sm">
                    Sign out
                  </button>
                </form>
              </div>
              <AccountDashboard email={session.user?.email ?? ""} />
            </>
          ) : (
            <SignInUI />
          )}
        </div>
      </div>
    </>
  );
}

function SignInUI() {
  return (
    <div style={{ maxWidth: "400px", margin: "0 auto" }}>
      <div className="content-block" style={{ textAlign: "center" }}>
        <div style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>🔐</div>
        <h2 style={{ marginBottom: "0.5rem" }}>Sign in</h2>
        <p style={{ fontSize: "0.9rem", color: "var(--ink-muted)", marginBottom: "1.75rem" }}>
          Connect your account to save products to your library and retrieve past purchases.
        </p>

        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          <form action={async () => { "use server"; await signIn("google", { redirectTo: "/account" }); }}>
            <button
              type="submit"
              style={{
                width: "100%",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                gap: "0.65rem",
                padding: "0.75rem 1.25rem",
                background: "white",
                border: "1px solid var(--border)",
                borderRadius: "var(--r)",
                fontSize: "0.9rem",
                fontWeight: 600,
                cursor: "pointer",
                color: "var(--ink)",
                transition: "border-color 0.15s, box-shadow 0.15s",
              }}
            >
              <GoogleIcon />
              Continue with Google
            </button>
          </form>

          <form action={async () => { "use server"; await signIn("github", { redirectTo: "/account" }); }}>
            <button
              type="submit"
              style={{
                width: "100%",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                gap: "0.65rem",
                padding: "0.75rem 1.25rem",
                background: "#24292e",
                border: "1px solid transparent",
                borderRadius: "var(--r)",
                fontSize: "0.9rem",
                fontWeight: 600,
                cursor: "pointer",
                color: "white",
                transition: "opacity 0.15s",
              }}
            >
              <GitHubIcon />
              Continue with GitHub
            </button>
          </form>
        </div>

        <p style={{ marginTop: "1.25rem", fontSize: "0.78rem", color: "var(--ink-subtle)" }}>
          No password needed. We only store your name and email from your provider.
        </p>
      </div>

      <div className="content-block" style={{ marginTop: "1rem", background: "var(--bg-alt)" }}>
        <p style={{ fontSize: "0.82rem", color: "var(--ink-muted)", marginBottom: "0.5rem" }}>
          <strong style={{ color: "var(--ink)" }}>Don&apos;t need an account?</strong>
        </p>
        <p style={{ fontSize: "0.82rem", color: "var(--ink-muted)" }}>
          You can browse free downloads without signing in. Past purchases can also be retrieved by email on this page after signing in.
        </p>
      </div>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
    </svg>
  );
}

function GitHubIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/>
    </svg>
  );
}
