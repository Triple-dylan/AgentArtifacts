# Audit Prep Prompt Pack
**Product ID:** `AA-PRM-FIN-AUDIT-BASE-071`  
...
Prompts for audit trail narration, control documentation, and auditor response drafts
...
```
Analyse the following audit prep in depth.
...
```
Process the following audit prep and return a structured JSON response.
...
```
Compare the following audit prep options and recommend the best choice.
...
```
Review the following audit prep and identify the top 5 improvements.
...
```
Generate a complete, professional audit prep for the following specification.
...
```
Validate the following audit prep against best practices and requirements.
...
```
Adapt the following audit prep for a different audience or context.
...
```
Summarise the following audit prep for an executive audience.
...
```
Review and improve the following audit prep draft.
...
```
Generate 15 clarifying questions about the following audit prep.
...
```
Prioritise the following audit prep items using the given criteria.
...
```
Write a professional report on the following audit prep.
...
```json
{
  "product_id": "AA-PRM-FIN-AUDIT-BASE-071",
  "slug": "prompt-audit-prep-base",
  "name": "Audit Prep Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-audit-prep-base.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
