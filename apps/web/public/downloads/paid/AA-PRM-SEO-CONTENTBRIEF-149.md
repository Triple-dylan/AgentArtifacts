# Content Brief Prompt Pack
**Product ID:** `AA-PRM-SEO-CONTENTBRIEF-149`  
...
Keyword-driven content brief prompts that cut research time and align writers with intent
...
```
Analyse the following content brief in depth.
...
```
Process the following content brief and return a structured JSON response.
...
```
Compare the following content brief options and recommend the best choice.
...
```
Review the following content brief and identify the top 5 improvements.
...
```
Generate a complete, professional content brief for the following specification.
...
```
Validate the following content brief against best practices and requirements.
...
```
Adapt the following content brief for a different audience or context.
...
```
Summarise the following content brief for an executive audience.
...
```
Review and improve the following content brief draft.
...
```
Generate 15 clarifying questions about the following content brief.
...
```
Prioritise the following content brief items using the given criteria.
...
```
Write a professional report on the following content brief.
...
```json
{
  "product_id": "AA-PRM-SEO-CONTENTBRIEF-149",
  "slug": "prompt-content-brief-pack",
  "name": "Content Brief Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-content-brief-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
