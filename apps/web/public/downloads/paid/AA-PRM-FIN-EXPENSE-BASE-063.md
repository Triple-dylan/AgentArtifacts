# Expense Report Prompt Pack
**Product ID:** `AA-PRM-FIN-EXPENSE-BASE-063`  
...
Prompt templates for categorizing expenses, drafting reports, and flagging anomalies
...
```
Analyse the following expense report in depth.
...
```
Process the following expense report and return a structured JSON response.
...
```
Compare the following expense report options and recommend the best choice.
...
```
Review the following expense report and identify the top 5 improvements.
...
```
Generate a complete, professional expense report for the following specification.
...
```
Validate the following expense report against best practices and requirements.
...
```
Adapt the following expense report for a different audience or context.
...
```
Summarise the following expense report for an executive audience.
...
```
Review and improve the following expense report draft.
...
```
Generate 15 clarifying questions about the following expense report.
...
```
Prioritise the following expense report items using the given criteria.
...
```
Write a professional report on the following expense report.
...
```json
{
  "product_id": "AA-PRM-FIN-EXPENSE-BASE-063",
  "slug": "prompt-expense-report-base",
  "name": "Expense Report Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-expense-report-base.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
