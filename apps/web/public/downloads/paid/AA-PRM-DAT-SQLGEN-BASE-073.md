# SQL Generation Prompt Pack
**Product ID:** `AA-PRM-DAT-SQLGEN-BASE-073`  
...
Natural language to SQL prompts for analysts, with validation and optimization hints
...
```
Analyse the following sql generation in depth.
...
```
Process the following sql generation and return a structured JSON response.
...
```
Compare the following sql generation options and recommend the best choice.
...
```
Review the following sql generation and identify the top 5 improvements.
...
```
Generate a complete, professional sql generation for the following specification.
...
```
Validate the following sql generation against best practices and requirements.
...
```
Adapt the following sql generation for a different audience or context.
...
```
Summarise the following sql generation for an executive audience.
...
```
Review and improve the following sql generation draft.
...
```
Generate 15 clarifying questions about the following sql generation.
...
```
Prioritise the following sql generation items using the given criteria.
...
```
Write a professional report on the following sql generation.
...
```json
{
  "product_id": "AA-PRM-DAT-SQLGEN-BASE-073",
  "slug": "prompt-sql-generation-base",
  "name": "SQL Generation Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-sql-generation-base.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
