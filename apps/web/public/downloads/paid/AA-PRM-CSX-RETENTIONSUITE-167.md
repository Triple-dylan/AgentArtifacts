# Retention Playbook Prompt Suite
**Product ID:** `AA-PRM-CSX-RETENTIONSUITE-167`  
...
Comprehensive prompt suite for churn signals, save plays, and renewal negotiation scripts
...
```
Analyse the following retention playbook in depth.
...
```
Process the following retention playbook and return a structured JSON response.
...
```
Compare the following retention playbook options and recommend the best choice.
...
```
Review the following retention playbook and identify the top 5 improvements.
...
```
Generate a complete, professional retention playbook for the following specification.
...
```
Validate the following retention playbook against best practices and requirements.
...
```
Adapt the following retention playbook for a different audience or context.
...
```
Summarise the following retention playbook for an executive audience.
...
```
Review and improve the following retention playbook draft.
...
```
Generate 15 clarifying questions about the following retention playbook.
...
```
Prioritise the following retention playbook items using the given criteria.
...
```
Write a professional report on the following retention playbook.
...
```json
{
  "product_id": "AA-PRM-CSX-RETENTIONSUITE-167",
  "slug": "prompt-retention-playbook-suite",
  "name": "Retention Playbook Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-retention-playbook-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
