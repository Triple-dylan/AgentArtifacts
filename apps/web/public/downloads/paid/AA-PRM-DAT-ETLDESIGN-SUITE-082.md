# ETL Design Prompt Suite
**Product ID:** `AA-PRM-DAT-ETLDESIGN-SUITE-082`  
...
Prompts for pipeline architecture, transformation logic, and data quality rule generation
...
```
Analyse the following etl design in depth.
...
```
Process the following etl design and return a structured JSON response.
...
```
Compare the following etl design options and recommend the best choice.
...
```
Review the following etl design and identify the top 5 improvements.
...
```
Generate a complete, professional etl design for the following specification.
...
```
Validate the following etl design against best practices and requirements.
...
```
Adapt the following etl design for a different audience or context.
...
```
Summarise the following etl design for an executive audience.
...
```
Review and improve the following etl design draft.
...
```
Generate 15 clarifying questions about the following etl design.
...
```
Prioritise the following etl design items using the given criteria.
...
```
Write a professional report on the following etl design.
...
```json
{
  "product_id": "AA-PRM-DAT-ETLDESIGN-SUITE-082",
  "slug": "prompt-etl-design-suite",
  "name": "ETL Design Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-etl-design-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
