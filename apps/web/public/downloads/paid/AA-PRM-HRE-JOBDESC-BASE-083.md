# Job Description Prompt Pack
**Product ID:** `AA-PRM-HRE-JOBDESC-BASE-083`  
...
Prompt templates for writing inclusive, structured job descriptions by role and level
...
```
Analyse the following job description in depth.
...
```
Process the following job description and return a structured JSON response.
...
```
Compare the following job description options and recommend the best choice.
...
```
Review the following job description and identify the top 5 improvements.
...
```
Generate a complete, professional job description for the following specification.
...
```
Validate the following job description against best practices and requirements.
...
```
Adapt the following job description for a different audience or context.
...
```
Summarise the following job description for an executive audience.
...
```
Review and improve the following job description draft.
...
```
Generate 15 clarifying questions about the following job description.
...
```
Prioritise the following job description items using the given criteria.
...
```
Write a professional report on the following job description.
...
```json
{
  "product_id": "AA-PRM-HRE-JOBDESC-BASE-083",
  "slug": "prompt-job-description-base",
  "name": "Job Description Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-job-description-base.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
