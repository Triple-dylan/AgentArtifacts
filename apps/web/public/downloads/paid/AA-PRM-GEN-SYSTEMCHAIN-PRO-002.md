...
Multi-role chain prompts for workflows
...
```
Analyse the following system chain in depth.
...
```
Process the following system chain and return a structured JSON response.
...
```
Compare the following system chain options and recommend the best choice.
...
```
Review the following system chain and identify the top 5 improvements.
...
```
Generate a complete, professional system chain for the following specification.
...
```
Validate the following system chain against best practices and requirements.
...
```
Adapt the following system chain for a different audience or context.
...
```
Summarise the following system chain for an executive audience.
...
```
Review and improve the following system chain draft.
...
```
Generate 15 clarifying questions about the following system chain.
...
```
Prioritise the following system chain items using the given criteria.
...
```
Write a professional report on the following system chain.
...
```json
{
  "product_id": "AA-PRM-GEN-SYSTEMCHAIN-PRO-002",
  "slug": "prompt-system-chain",
  "name": "System Chain Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-system-chain.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
