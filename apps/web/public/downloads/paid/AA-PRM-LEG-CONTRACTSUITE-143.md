# Contract Lifecycle Prompt Suite
**Product ID:** `AA-PRM-LEG-CONTRACTSUITE-143`  
...
Full suite from initial term sheet through redline, execution, and renewal prompts
...
```
Analyse the following contract lifecycle in depth.
...
```
Process the following contract lifecycle and return a structured JSON response.
...
```
Compare the following contract lifecycle options and recommend the best choice.
...
```
Review the following contract lifecycle and identify the top 5 improvements.
...
```
Generate a complete, professional contract lifecycle for the following specification.
...
```
Validate the following contract lifecycle against best practices and requirements.
...
```
Adapt the following contract lifecycle for a different audience or context.
...
```
Summarise the following contract lifecycle for an executive audience.
...
```
Review and improve the following contract lifecycle draft.
...
```
Generate 15 clarifying questions about the following contract lifecycle.
...
```
Prioritise the following contract lifecycle items using the given criteria.
...
```
Write a professional report on the following contract lifecycle.
...
```json
{
  "product_id": "AA-PRM-LEG-CONTRACTSUITE-143",
  "slug": "prompt-contract-lifecycle-suite",
  "name": "Contract Lifecycle Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-contract-lifecycle-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
