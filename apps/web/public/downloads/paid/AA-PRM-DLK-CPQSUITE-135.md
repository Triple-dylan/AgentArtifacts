# CPQ Workflow Prompt Suite
**Product ID:** `AA-PRM-DLK-CPQSUITE-135`  
...
End-to-end prompt suite for configure-price-quote workflows from RFP to contract redline
...
```
Analyse the following cpq workflow in depth.
...
```
Process the following cpq workflow and return a structured JSON response.
...
```
Compare the following cpq workflow options and recommend the best choice.
...
```
Review the following cpq workflow and identify the top 5 improvements.
...
```
Generate a complete, professional cpq workflow for the following specification.
...
```
Validate the following cpq workflow against best practices and requirements.
...
```
Adapt the following cpq workflow for a different audience or context.
...
```
Summarise the following cpq workflow for an executive audience.
...
```
Review and improve the following cpq workflow draft.
...
```
Generate 15 clarifying questions about the following cpq workflow.
...
```
Prioritise the following cpq workflow items using the given criteria.
...
```
Write a professional report on the following cpq workflow.
...
```json
{
  "product_id": "AA-PRM-DLK-CPQSUITE-135",
  "slug": "prompt-cpq-workflow-suite",
  "name": "CPQ Workflow Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-cpq-workflow-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
