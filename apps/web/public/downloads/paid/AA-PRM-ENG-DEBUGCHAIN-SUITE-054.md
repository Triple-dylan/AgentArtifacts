# Debug & Root Cause Prompt Suite
**Product ID:** `AA-PRM-ENG-DEBUGCHAIN-SUITE-054`  
...
Chained prompts for systematic debugging, error tracing, and fix generation
...
```
Analyse the following debug & root cause in depth.
...
```
Process the following debug & root cause and return a structured JSON response.
...
```
Compare the following debug & root cause options and recommend the best choice.
...
```
Review the following debug & root cause and identify the top 5 improvements.
...
```
Generate a complete, professional debug & root cause for the following specification.
...
```
Validate the following debug & root cause against best practices and requirements.
...
```
Adapt the following debug & root cause for a different audience or context.
...
```
Summarise the following debug & root cause for an executive audience.
...
```
Review and improve the following debug & root cause draft.
...
```
Generate 15 clarifying questions about the following debug & root cause.
...
```
Prioritise the following debug & root cause items using the given criteria.
...
```
Write a professional report on the following debug & root cause.
...
```json
{
  "product_id": "AA-PRM-ENG-DEBUGCHAIN-SUITE-054",
  "slug": "prompt-debug-chain-suite",
  "name": "Debug & Root Cause Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-debug-chain-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
