# Architecture Review Prompt Suite
**Product ID:** `AA-PRM-ENG-ARCHITECTURE-SUITE-062`  
...
Prompt chain for system design reviews, ADR generation, and trade-off analysis
...
```
Analyse the following architecture review in depth.
...
```
Process the following architecture review and return a structured JSON response.
...
```
Compare the following architecture review options and recommend the best choice.
...
```
Review the following architecture review and identify the top 5 improvements.
...
```
Generate a complete, professional architecture review for the following specification.
...
```
Validate the following architecture review against best practices and requirements.
...
```
Adapt the following architecture review for a different audience or context.
...
```
Summarise the following architecture review for an executive audience.
...
```
Review and improve the following architecture review draft.
...
```
Generate 15 clarifying questions about the following architecture review.
...
```
Prioritise the following architecture review items using the given criteria.
...
```
Write a professional report on the following architecture review.
...
```json
{
  "product_id": "AA-PRM-ENG-ARCHITECTURE-SUITE-062",
  "slug": "prompt-architecture-suite",
  "name": "Architecture Review Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-architecture-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
