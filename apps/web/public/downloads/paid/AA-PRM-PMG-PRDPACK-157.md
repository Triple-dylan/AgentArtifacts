# PRD Writing Prompt Pack
**Product ID:** `AA-PRM-PMG-PRDPACK-157`  
...
Structured prompts for writing crisp PRDs, acceptance criteria, and edge case tables
...
```
Analyse the following prd writing in depth.
...
```
Process the following prd writing and return a structured JSON response.
...
```
Compare the following prd writing options and recommend the best choice.
...
```
Review the following prd writing and identify the top 5 improvements.
...
```
Generate a complete, professional prd writing for the following specification.
...
```
Validate the following prd writing against best practices and requirements.
...
```
Adapt the following prd writing for a different audience or context.
...
```
Summarise the following prd writing for an executive audience.
...
```
Review and improve the following prd writing draft.
...
```
Generate 15 clarifying questions about the following prd writing.
...
```
Prioritise the following prd writing items using the given criteria.
...
```
Write a professional report on the following prd writing.
...
```json
{
  "product_id": "AA-PRM-PMG-PRDPACK-157",
  "slug": "prompt-prd-writing-pack",
  "name": "PRD Writing Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-prd-writing-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
