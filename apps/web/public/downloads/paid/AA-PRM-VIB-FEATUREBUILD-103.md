# Feature Build Prompt Suite
**Product ID:** `AA-PRM-VIB-FEATUREBUILD-103`  
...
Full lifecycle prompts from spec to implementation and test generation for LLM-driven dev
...
```
Analyse the following feature build in depth.
...
```
Process the following feature build and return a structured JSON response.
...
```
Compare the following feature build options and recommend the best choice.
...
```
Review the following feature build and identify the top 5 improvements.
...
```
Generate a complete, professional feature build for the following specification.
...
```
Validate the following feature build against best practices and requirements.
...
```
Adapt the following feature build for a different audience or context.
...
```
Summarise the following feature build for an executive audience.
...
```
Review and improve the following feature build draft.
...
```
Generate 15 clarifying questions about the following feature build.
...
```
Prioritise the following feature build items using the given criteria.
...
```
Write a professional report on the following feature build.
...
```json
{
  "product_id": "AA-PRM-VIB-FEATUREBUILD-103",
  "slug": "prompt-feature-build-suite",
  "name": "Feature Build Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-feature-build-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "cursor ",
