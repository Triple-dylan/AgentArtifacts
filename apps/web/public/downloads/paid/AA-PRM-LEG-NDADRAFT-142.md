...
**Product ID:** `AA-PRM-LEG-NDADRAFT-142`  
...
Prompt templates for drafting mutual and one-way NDAs with jurisdiction-aware fallback clauses
...
```
Analyse the following nda drafting in depth.
...
```
Process the following nda drafting and return a structured JSON response.
...
```
Compare the following nda drafting options and recommend the best choice.
...
```
Review the following nda drafting and identify the top 5 improvements.
...
```
Generate a complete, professional nda drafting for the following specification.
...
```
Validate the following nda drafting against best practices and requirements.
...
```
Adapt the following nda drafting for a different audience or context.
...
```
Summarise the following nda drafting for an executive audience.
...
```
Review and improve the following nda drafting draft.
...
```
Generate 15 clarifying questions about the following nda drafting.
...
```
Prioritise the following nda drafting items using the given criteria.
...
```
Write a professional report on the following nda drafting.
...
```json
{
  "product_id": "AA-PRM-LEG-NDADRAFT-142",
  "slug": "prompt-nda-drafting-pack",
  "name": "NDA Drafting Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-nda-drafting-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
