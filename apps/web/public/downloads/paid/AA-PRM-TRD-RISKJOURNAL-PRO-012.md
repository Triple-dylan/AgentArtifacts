# Risk Journal Prompt Pack
**Product ID:** `AA-PRM-TRD-RISKJOURNAL-PRO-012`  
...
Prompts for risk review and journaling
...
```
Analyse the following risk journal in depth.
...
```
Process the following risk journal and return a structured JSON response.
...
```
Compare the following risk journal options and recommend the best choice.
...
```
Review the following risk journal and identify the top 5 improvements.
...
```
Generate a complete, professional risk journal for the following specification.
...
```
Validate the following risk journal against best practices and requirements.
...
```
Adapt the following risk journal for a different audience or context.
...
```
Summarise the following risk journal for an executive audience.
...
```
Review and improve the following risk journal draft.
...
```
Generate 15 clarifying questions about the following risk journal.
...
```
Prioritise the following risk journal items using the given criteria.
...
```
Write a professional report on the following risk journal.
...
```json
{
  "product_id": "AA-PRM-TRD-RISKJOURNAL-PRO-012",
  "slug": "prompt-trading-risk-journal",
  "name": "Risk Journal Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-trading-risk-journal.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
