...
**Product ID:** `AA-PRM-GEN-IDEATION-BASE-001`  
...
Fast ideation prompts for agent tasks
...
```
Analyse the following ideation in depth.
...
```
Process the following ideation and return a structured JSON response.
...
```
Compare the following ideation options and recommend the best choice.
...
```
Review the following ideation and identify the top 5 improvements.
...
```
Generate a complete, professional ideation for the following specification.
...
```
Validate the following ideation against best practices and requirements.
...
```
Adapt the following ideation for a different audience or context.
...
```
Summarise the following ideation for an executive audience.
...
```
Review and improve the following ideation draft.
...
```
Generate 15 clarifying questions about the following ideation.
...
```
Prioritise the following ideation items using the given criteria.
...
```
Write a professional report on the following ideation.
...
```json
{
  "product_id": "AA-PRM-GEN-IDEATION-BASE-001",
  "slug": "prompt-ideation-base",
  "name": "Ideation Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-ideation-base.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
