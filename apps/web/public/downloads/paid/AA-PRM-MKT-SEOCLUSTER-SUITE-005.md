# SEO Cluster Prompt Suite
**Product ID:** `AA-PRM-MKT-SEOCLUSTER-SUITE-005`  
...
SEO planning prompts with cluster maps
...
```
Analyse the following seo cluster in depth.
...
```
Process the following seo cluster and return a structured JSON response.
...
```
Compare the following seo cluster options and recommend the best choice.
...
```
Review the following seo cluster and identify the top 5 improvements.
...
```
Generate a complete, professional seo cluster for the following specification.
...
```
Validate the following seo cluster against best practices and requirements.
...
```
Adapt the following seo cluster for a different audience or context.
...
```
Summarise the following seo cluster for an executive audience.
...
```
Review and improve the following seo cluster draft.
...
```
Generate 15 clarifying questions about the following seo cluster.
...
```
Prioritise the following seo cluster items using the given criteria.
...
```
Write a professional report on the following seo cluster.
...
```json
{
  "product_id": "AA-PRM-MKT-SEOCLUSTER-SUITE-005",
  "slug": "prompt-seo-cluster-suite",
  "name": "SEO Cluster Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-seo-cluster-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
