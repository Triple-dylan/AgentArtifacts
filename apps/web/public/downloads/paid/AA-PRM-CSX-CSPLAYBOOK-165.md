# CS Playbook Prompt Pack
**Product ID:** `AA-PRM-CSX-CSPLAYBOOK-165`  
...
30 prompts for building customer success playbooks covering onboarding and expansion motions
...
```
Analyse the following cs playbook in depth.
...
```
Process the following cs playbook and return a structured JSON response.
...
```
Compare the following cs playbook options and recommend the best choice.
...
```
Review the following cs playbook and identify the top 5 improvements.
...
```
Generate a complete, professional cs playbook for the following specification.
...
```
Validate the following cs playbook against best practices and requirements.
...
```
Adapt the following cs playbook for a different audience or context.
...
```
Summarise the following cs playbook for an executive audience.
...
```
Review and improve the following cs playbook draft.
...
```
Generate 15 clarifying questions about the following cs playbook.
...
```
Prioritise the following cs playbook items using the given criteria.
...
```
Write a professional report on the following cs playbook.
...
```json
{
  "product_id": "AA-PRM-CSX-CSPLAYBOOK-165",
  "slug": "prompt-cs-playbook-pack",
  "name": "CS Playbook Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-cs-playbook-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
