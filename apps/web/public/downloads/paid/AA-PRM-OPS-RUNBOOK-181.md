# Runbook Generator Prompt Pack
**Product ID:** `AA-PRM-OPS-RUNBOOK-181`  
...
Prompts for auto-generating runbooks for common SRE incidents and scheduled maintenance
...
```
Analyse the following runbook generator in depth.
...
```
Process the following runbook generator and return a structured JSON response.
...
```
Compare the following runbook generator options and recommend the best choice.
...
```
Review the following runbook generator and identify the top 5 improvements.
...
```
Generate a complete, professional runbook generator for the following specification.
...
```
Validate the following runbook generator against best practices and requirements.
...
```
Adapt the following runbook generator for a different audience or context.
...
```
Summarise the following runbook generator for an executive audience.
...
```
Review and improve the following runbook generator draft.
...
```
Generate 15 clarifying questions about the following runbook generator.
...
```
Prioritise the following runbook generator items using the given criteria.
...
```
Write a professional report on the following runbook generator.
...
```json
{
  "product_id": "AA-PRM-OPS-RUNBOOK-181",
  "slug": "prompt-runbook-generator-pack",
  "name": "Runbook Generator Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-runbook-generator-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
