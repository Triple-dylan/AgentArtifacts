...
**Product ID:** `AA-SKL-WEB-SCHEMADESIGN-112`  
...
Generates normalized DB schemas from domain descriptions with migration scripts

This skill module provides a standardised interface for integrating AI-powered
`database schema designer` capabilities into any agent workflow.
...
```markdown
# Skill: Database Schema Designer Skill v1.0
...
```
You are a specialised AI skill module for database schema designer.
...
SKILL_SYSTEM_PROMPT = """You are a specialised AI skill module for database schema designer.
...
Save as `.claude/skills/skill-db-schema-designer.md` and invoke with `/db-schema-designer`
...
```json
{
  "product_id": "AA-SKL-WEB-SCHEMADESIGN-112",
  "skill_name": "Database Schema Designer Skill",
  "version": "1.0.0",
  "category": "skill",
