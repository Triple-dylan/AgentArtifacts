...
**Product ID:** `AA-PRM-VIB-CURSORRULES-101`  
...
30 ready-to-use .cursorrules prompt templates to supercharge AI-assisted coding
...
```
Analyse the following cursor rules in depth.
...
```
Process the following cursor rules and return a structured JSON response.
...
```
Compare the following cursor rules options and recommend the best choice.
...
```
Review the following cursor rules and identify the top 5 improvements.
...
```
Generate a complete, professional cursor rules for the following specification.
...
```
Validate the following cursor rules against best practices and requirements.
...
```
Adapt the following cursor rules for a different audience or context.
...
```
Summarise the following cursor rules for an executive audience.
...
```
Review and improve the following cursor rules draft.
...
```
Generate 15 clarifying questions about the following cursor rules.
...
```
Prioritise the following cursor rules items using the given criteria.
...
```
Write a professional report on the following cursor rules.
...
```json
{
  "product_id": "AA-PRM-VIB-CURSORRULES-101",
  "slug": "prompt-cursor-rules-pack",
  "name": "Cursor Rules Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-cursor-rules-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "cursor ",
