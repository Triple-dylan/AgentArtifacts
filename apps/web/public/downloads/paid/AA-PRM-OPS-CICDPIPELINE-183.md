# CI/CD Pipeline Prompt Suite
**Product ID:** `AA-PRM-OPS-CICDPIPELINE-183`  
...
Prompt suite for designing, auditing, and optimizing CI/CD pipelines in GitHub Actions and GitLab
...
```
Analyse the following ci/cd pipeline in depth.
...
```
Process the following ci/cd pipeline and return a structured JSON response.
...
```
Compare the following ci/cd pipeline options and recommend the best choice.
...
```
Review the following ci/cd pipeline and identify the top 5 improvements.
...
```
Generate a complete, professional ci/cd pipeline for the following specification.
...
```
Validate the following ci/cd pipeline against best practices and requirements.
...
```
Adapt the following ci/cd pipeline for a different audience or context.
...
```
Summarise the following ci/cd pipeline for an executive audience.
...
```
Review and improve the following ci/cd pipeline draft.
...
```
Generate 15 clarifying questions about the following ci/cd pipeline.
...
```
Prioritise the following ci/cd pipeline items using the given criteria.
...
```
Write a professional report on the following ci/cd pipeline.
...
```json
{
  "product_id": "AA-PRM-OPS-CICDPIPELINE-183",
  "slug": "prompt-cicd-pipeline-suite",
  "name": "CI/CD Pipeline Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-cicd-pipeline-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
