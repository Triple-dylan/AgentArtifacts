# FP&A Planning Prompt Suite
**Product ID:** `AA-PRM-FIN2-FPASUITE-127`  
...
Comprehensive prompt suite for budget cycles, variance analysis, and rolling forecasts
...
```
Analyse the following fp&a planning in depth.
...
```
Process the following fp&a planning and return a structured JSON response.
...
```
Compare the following fp&a planning options and recommend the best choice.
...
```
Review the following fp&a planning and identify the top 5 improvements.
...
```
Generate a complete, professional fp&a planning for the following specification.
...
```
Validate the following fp&a planning against best practices and requirements.
...
```
Adapt the following fp&a planning for a different audience or context.
...
```
Summarise the following fp&a planning for an executive audience.
...
```
Review and improve the following fp&a planning draft.
...
```
Generate 15 clarifying questions about the following fp&a planning.
...
```
Prioritise the following fp&a planning items using the given criteria.
...
```
Write a professional report on the following fp&a planning.
...
```json
{
  "product_id": "AA-PRM-FIN2-FPASUITE-127",
  "slug": "prompt-fpa-planning-suite",
  "name": "FP&A Planning Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-fpa-planning-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
