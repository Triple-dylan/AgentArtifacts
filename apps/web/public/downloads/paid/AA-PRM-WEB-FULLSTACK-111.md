# Full-Stack Build Prompt Suite
**Product ID:** `AA-PRM-WEB-FULLSTACK-111`  
...
End-to-end prompts covering auth, data layer, API, and deployment config for web apps
...
```
Analyse the following full-stack build in depth.
...
```
Process the following full-stack build and return a structured JSON response.
...
```
Compare the following full-stack build options and recommend the best choice.
...
```
Review the following full-stack build and identify the top 5 improvements.
...
```
Generate a complete, professional full-stack build for the following specification.
...
```
Validate the following full-stack build against best practices and requirements.
...
```
Adapt the following full-stack build for a different audience or context.
...
```
Summarise the following full-stack build for an executive audience.
...
```
Review and improve the following full-stack build draft.
...
```
Generate 15 clarifying questions about the following full-stack build.
...
```
Prioritise the following full-stack build items using the given criteria.
...
```
Write a professional report on the following full-stack build.
...
```json
{
  "product_id": "AA-PRM-WEB-FULLSTACK-111",
  "slug": "prompt-fullstack-build-suite",
  "name": "Full-Stack Build Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-fullstack-build-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
