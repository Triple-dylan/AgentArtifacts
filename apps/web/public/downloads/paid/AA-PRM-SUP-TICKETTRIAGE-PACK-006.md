# Ticket Triage Prompt Pack
**Product ID:** `AA-PRM-SUP-TICKETTRIAGE-PACK-006`  
...
Support ticket routing prompts
...
```
Analyse the following ticket triage in depth.
...
```
Process the following ticket triage and return a structured JSON response.
...
```
Compare the following ticket triage options and recommend the best choice.
...
```
Review the following ticket triage and identify the top 5 improvements.
...
```
Generate a complete, professional ticket triage for the following specification.
...
```
Validate the following ticket triage against best practices and requirements.
...
```
Adapt the following ticket triage for a different audience or context.
...
```
Summarise the following ticket triage for an executive audience.
...
```
Review and improve the following ticket triage draft.
...
```
Generate 15 clarifying questions about the following ticket triage.
...
```
Prioritise the following ticket triage items using the given criteria.
...
```
Write a professional report on the following ticket triage.
...
```json
{
  "product_id": "AA-PRM-SUP-TICKETTRIAGE-PACK-006",
  "slug": "prompt-ticket-triage-pack",
  "name": "Ticket Triage Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-ticket-triage-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
