# Event Driven Signal Prompt Suite
**Product ID:** `AA-PRM-TRD-EVENTSIGNAL-PRO-010`  
...
Prompts for event based signal extraction
...
```
Analyse the following event driven signal in depth.
...
```
Process the following event driven signal and return a structured JSON response.
...
```
Compare the following event driven signal options and recommend the best choice.
...
```
Review the following event driven signal and identify the top 5 improvements.
...
```
Generate a complete, professional event driven signal for the following specification.
...
```
Validate the following event driven signal against best practices and requirements.
...
```
Adapt the following event driven signal for a different audience or context.
...
```
Summarise the following event driven signal for an executive audience.
...
```
Review and improve the following event driven signal draft.
...
```
Generate 15 clarifying questions about the following event driven signal.
...
```
Prioritise the following event driven signal items using the given criteria.
...
```
Write a professional report on the following event driven signal.
...
```json
{
  "product_id": "AA-PRM-TRD-EVENTSIGNAL-PRO-010",
  "slug": "prompt-trading-event-signal",
  "name": "Event Driven Signal Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-trading-event-signal.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
