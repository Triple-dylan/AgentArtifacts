# Dashboard Brief Prompt Suite
**Product ID:** `AA-PRM-DAT-DASHBOARD-SUITE-074`  
...
Chained prompts for generating executive dashboard summaries and data narratives
...
```
Analyse the following dashboard brief in depth.
...
```
Process the following dashboard brief and return a structured JSON response.
...
```
Compare the following dashboard brief options and recommend the best choice.
...
```
Review the following dashboard brief and identify the top 5 improvements.
...
```
Generate a complete, professional dashboard brief for the following specification.
...
```
Validate the following dashboard brief against best practices and requirements.
...
```
Adapt the following dashboard brief for a different audience or context.
...
```
Summarise the following dashboard brief for an executive audience.
...
```
Review and improve the following dashboard brief draft.
...
```
Generate 15 clarifying questions about the following dashboard brief.
...
```
Prioritise the following dashboard brief items using the given criteria.
...
```
Write a professional report on the following dashboard brief.
...
```json
{
  "product_id": "AA-PRM-DAT-DASHBOARD-SUITE-074",
  "slug": "prompt-dashboard-brief-suite",
  "name": "Dashboard Brief Prompt Suite",
  "category": "prompt",
  "subcategory": "prompt_suite",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-dashboard-brief-suite.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
