# Backtest Brief Prompt Pack
**Product ID:** `AA-PRM-TRD2-BACKTEST-BRIEF-094`  
...
Structured prompts for backtesting analysis summaries and performance critique
...
```
Analyse the following backtest brief in depth.
...
```
Process the following backtest brief and return a structured JSON response.
...
```
Compare the following backtest brief options and recommend the best choice.
...
```
Review the following backtest brief and identify the top 5 improvements.
...
```
Generate a complete, professional backtest brief for the following specification.
...
```
Validate the following backtest brief against best practices and requirements.
...
```
Adapt the following backtest brief for a different audience or context.
...
```
Summarise the following backtest brief for an executive audience.
...
```
Review and improve the following backtest brief draft.
...
```
Generate 15 clarifying questions about the following backtest brief.
...
```
Prioritise the following backtest brief items using the given criteria.
...
```
Write a professional report on the following backtest brief.
...
```json
{
  "product_id": "AA-PRM-TRD2-BACKTEST-BRIEF-094",
  "slug": "prompt-backtest-brief-pack",
  "name": "Backtest Brief Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-backtest-brief-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
