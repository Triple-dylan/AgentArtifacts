# Incident Postmortem Prompt Pack
**Product ID:** `AA-PRM-OPS-POSTMORTEM-182`  
...
Structured prompts for blameless postmortems covering timeline, impact, and action items
...
```
Analyse the following incident postmortem in depth.
...
```
Process the following incident postmortem and return a structured JSON response.
...
```
Compare the following incident postmortem options and recommend the best choice.
...
```
Review the following incident postmortem and identify the top 5 improvements.
...
```
Generate a complete, professional incident postmortem for the following specification.
...
```
Validate the following incident postmortem against best practices and requirements.
...
```
Adapt the following incident postmortem for a different audience or context.
...
```
Summarise the following incident postmortem for an executive audience.
...
```
Review and improve the following incident postmortem draft.
...
```
Generate 15 clarifying questions about the following incident postmortem.
...
```
Prioritise the following incident postmortem items using the given criteria.
...
```
Write a professional report on the following incident postmortem.
...
```json
{
  "product_id": "AA-PRM-OPS-POSTMORTEM-182",
  "slug": "prompt-postmortem-pack",
  "name": "Incident Postmortem Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-postmortem-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
