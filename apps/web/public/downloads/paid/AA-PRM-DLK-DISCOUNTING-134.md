# Discounting Strategy Prompt Pack
**Product ID:** `AA-PRM-DLK-DISCOUNTING-134`  
...
Prompts for structuring discount approval rationale, thresholds, and counter-offer logic
...
```
Analyse the following discounting strategy in depth.
...
```
Process the following discounting strategy and return a structured JSON response.
...
```
Compare the following discounting strategy options and recommend the best choice.
...
```
Review the following discounting strategy and identify the top 5 improvements.
...
```
Generate a complete, professional discounting strategy for the following specification.
...
```
Validate the following discounting strategy against best practices and requirements.
...
```
Adapt the following discounting strategy for a different audience or context.
...
```
Summarise the following discounting strategy for an executive audience.
...
```
Review and improve the following discounting strategy draft.
...
```
Generate 15 clarifying questions about the following discounting strategy.
...
```
Prioritise the following discounting strategy items using the given criteria.
...
```
Write a professional report on the following discounting strategy.
...
```json
{
  "product_id": "AA-PRM-DLK-DISCOUNTING-134",
  "slug": "prompt-discounting-strategy-pack",
  "name": "Discounting Strategy Prompt Pack",
  "category": "prompt",
  "subcategory": "prompt_pack",
  "version": "1.0.0",
  "license": "commercial-single-seat",
  "files": [
    "prompt-discounting-strategy-pack.md",
    "manifest.json"
  ],
  "compatibility": [
    "openai ",
